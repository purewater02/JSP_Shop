...
<script type="text/javascript">
	function check(no) {
		...
		if(res) {
			/* let pwd = prompt('글 비밀번호를 입력해주세요.', '비밀번호'); */
			location.href="admin_qna_delete.do?no="+no;/* "&pwd="+pwd; */
		}
	}
</script>
</head>
<body>
	...
	<div align="center">
		...
		<table class="table_cont" border="1" cellspacing="0" width="500">
			<tr bgcolor="#eeffee">
				<th>글 번호</th> <th>글 제목</th> <th>작성자</th> <th>작성일</th> <th>답변  | 삭제</th>
			</tr>
			...
			<c:if test="${!empty list }">
				<c:forEach items="${list }" var="dto">
					<tr>
						...
						<a href="admin_qna_detail.do?no=${dto.getbNo()}&page=${paging.getPage()}">${dto.getbTitle() }</a></td>
						<td>${dto.getbWriter() }</td>
						<td>${dto.getbDate().substring(0,10) }</td>
						<td>
						<input type="button" value="답변" onclick="location.href='admin_qna_reply.do?no=${dto.getbNo() }&page=${paging.getPage()}'">
	...
	<c:url var="loadList" value="/admin_qna_list.do" />
