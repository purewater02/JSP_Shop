...
<body>
	...
	<div align="center" class="allList">
		...
		<c:if test="${page > block }">	<!-- 현재 페이지가 페이지 아래 보여지는 페이지 최대 수 보다 크다면, -->
			<a href="user_product_list_ctg.do?page=1&ctg=${ctg }">◀◀</a>	<!-- 맨 첫 페이지로 이동 -->
			&nbsp;
			<a href="user_product_list_ctg.do?page=${startBlock - 1}&ctg=${ctg }">◀</a>	 <!-- 현재 블럭 - 1 -->
		</c:if>
		
		
		<c:forEach begin="${startBlock }" end="${endBlock }" var="i">
			<c:if test="${i == page }">
				<b><a href="user_product_list_ctg.do?page=${i}&ctg=${ctg }">[${i }]</a></b>
			</c:if>
			
			<c:if test="${i != page }">
				<a href="user_product_list_ctg.do?page=${i}&ctg=${ctg }">[${i }]</a>
			</c:if>
		</c:forEach>
		
		
		<c:if test="${endBlock < allPage }">	<!-- 4번 페이지에서 보이는 끝 페이지 번호인 6보다 모든 페이지 수가 크다면 -->
			<a href="user_product_list_ctg.do?page=${endBlock + 1}&ctg=${ctg }">▶</a>	<!-- 맨 끝 페이지로 이동 -->
			&nbsp;
			<a href="user_product_list_ctg.do?page=${allPage}&ctg=${ctg }">▶▶</a>	 <!-- 현재 블럭 - 1 -->
		</c:if>
	</div>
